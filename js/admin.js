...
async function fetchDataAndUpdateCounts() {
  try {
    // Fetch account data
    const accountResponse = await axios.get("http://localhost:3001/api/auth", {
    ...
    const enrollmentResponse = await axios.get("http://localhost:3001/api/enrollees", {
      headers: { Authorization: "Bearer " + token },
    });
    enroleesCount = enrollmentResponse.data.length;
    pendingCount = enrollmentResponse.data.filter((pending) => !pending.isApproved).length;

    // Fetch pending request id data
    const requestIdResponse = await axios.get("http://localhost:3001/api/requestId", {
      headers: { Authorization: "Bearer " + token },
    });
    pendingRequestIdCount = requestIdResponse.data.filter((pending) => pending.isApproved==null).length;
